...
public final class ABCDGuesser2 {
    ...
    private static double getPositiveDoubleNotOne(SimpleReader in,
        ...
        while (num <= 0 || num == 1.0) {
            out.print("Enter a positive real number not equal to 1: ");
            String input = in.nextLine();
            if (FormatChecker.canParseDouble(input)) {
                num = Double.parseDouble(input);
                if (num <= 0 || num == 1) {
    ...
    /**
     * Calculates the relative error for the current estimate of mu.
     *
     * @param percentConv
     *            the double for the percent that makes up a whole
     * @param estimate
     *            current estimate to calculate relative error of
     * @param mu
     *            constant that algorithm is attempting to estimate
     * @return the relative error, calculated by 100% * |e - u| / u
     */
    private static double getRelError(double percentConv, double estimate,
            double mu) {
        double relError = percentConv * Math.abs(estimate - mu) / mu;
        return relError;
    }

    /**
     * Main method, uses getPositiveDouble and getPositiveDoubleNotOne to accept
     * user input, then applies the de Jager formula, calling getRelError to
     * calculate the relative error with each iteration.
     *
     * @param args
    ...
    public static void main(String[] args) {
         ...
         * error
         */
        for (int i = 0; i < charmingNums.length; i++) {
            for (int j = 0; j < charmingNums.length; j++) {
                for (int k = 0; k < charmingNums.length; k++) {
                    for (int l = 0; l < charmingNums.length; l++) {
                        estimate = Math.pow(w, charmingNums[i])
                                * Math.pow(x, charmingNums[j])
                                * Math.pow(y, charmingNums[k])
                                * Math.pow(z, charmingNums[l]);
                        relError = getRelError(percentConv, estimate, mu);
