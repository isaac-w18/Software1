...
public final class CryptoUtilities {
    ...
    public static void reduceToGCD(NaturalNumber n, NaturalNumber m) {
        ...
        if (!m.isZero()) {
            ...
            // clearing m and making n equal to the GCD
            n.transferFrom(m);
        }
    }
    ...
    public static boolean isEven(NaturalNumber n) {
        ...
        // looking at only the last digit
    ...
    public static void powerMod(NaturalNumber n, NaturalNumber p,
        ...
        // used to restore p later
        NaturalNumber pOriginal = new NaturalNumber2(p);
        ...
        if (!p.isZero()) {
            ...
            // if p is even, the only action needed is to multiply the two
            // results of powerMod by each other
            if (isEven(p)) {
                p.divide(two);
                powerMod(n2, p, m);
                powerMod(n, p, m);
                ...
                // if p is odd, also multiply by nOriginal
                p.divide(two);
                powerMod(n2, p, m);
                powerMod(n, p, m);
                n.multiply(n2);
    ...
    public static boolean isWitnessToCompositeness(NaturalNumber w,
        ...
        // nMinus1 to be used for powerMod
        NaturalNumber nMinus1 = new NaturalNumber2(n);
        nMinus1.decrement();

        // making copies and originals
        NaturalNumber wOriginal = new NaturalNumber2(w);
        NaturalNumber wCopy = new NaturalNumber2(w);
        NaturalNumber two = new NaturalNumber2(2);

        // w^2 mod n (if its a witness, it should equal 1)
        powerMod(wCopy, two, n);

        // w^n-1 mod n (if its a witness, it should NOT equal 1)
        powerMod(w, nMinus1, n);
        NaturalNumber shouldNotEqualOne = w.newInstance();
        shouldNotEqualOne.transferFrom(w);

        // restoring w
        w.transferFrom(wOriginal);

        NaturalNumber one = new NaturalNumber2(1);

        // both must be true for this to be a witness to compositeness
        return (wCopy.compareTo(one) == 0)
                || (shouldNotEqualOne.compareTo(one) != 0);
    }
    ...
    public static boolean isPrime2(NaturalNumber n) {
        ...
        // isPrime must be true if n < 4
        if (n.compareTo(four) >= 0) {

            // nMinusFour so that candidate can be incremented to be always > 1,
            // and so that n > candidate + 1, thus satisfying
            // isWitnessToCompositness' preconditions
            NaturalNumber nMinusFour = new NaturalNumber2(n);
            nMinusFour.subtract(four);

            // generating 50 candidates to check, or until isPrime = false
            final int numCandidates = 50;
            int i = 0;
            while (i < numCandidates && isPrime) {
                NaturalNumber candidate = randomNumber(nMinusFour);
                candidate.increment();
                candidate.increment();
    ...
    public static void generateNextLikelyPrime(NaturalNumber n) {
        ...
        /*
         * Using isPrime2 to check numbers, starting at n and increasing through
         * odd numbers only until n is likely prime
         */
        if (isEven(n)) {
            n.increment();
        }
