...
public final class StringReassembly {
    ...
    public static String combination(String str1, String str2, int overlap) {
        ...
        // return the combination of str1 and str2
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet,
        ...
        // initializing vars
        boolean addStr = true;
        String strToRemove = new String();

        // iterating through strSet
        for (String x : strSet) {
            if (addStr) {
                // if str is a substring of any element in strSet, do not add str
                if (x.contains(str)) {
                    addStr = false;
                    // if x is substring of str, remove x and later add str
                } else if (!str.equals(x) && str.contains(x)) {
                    strToRemove = x;
                }
            }
        }

        // modifying str based on info found in the loop
        if (!strToRemove.equals("")) {
            strSet.remove(strToRemove);
        }
        if (addStr) {
            strSet.add(str);
        }
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        // Iterate through until input is at the end
        while (!input.atEOS()) {
    ...
    public static void printWithLineSeparators(String text, SimpleWriter out) {
        ...
        // Iterates through, replacing all ~ with new lines
    ...
    public static void main(String[] args) {
        ...
        // PRINT GENOME TO FILE?

        /*
         * Get input file name
         */
        out.print("File name (with genome fragments): ");
        String inputFileName = in.nextLine();
        SimpleReader inFile = new SimpleReader1L(inputFileName);
        // ASK FOR DISORDER TO LOOK FOR
        ...
        // HERE, LOOK THROUGH FRAGMENTS FOR DISORDER
